...
class Ajax extends Backend
{
    ...
    public function getProvinces(){
        ...
        $provinces = Db::name('area')->where($where)->column('province_name','id');
        $this->success('', null, $provinces);
    }
    ...
    public function getCitys(){
        ...
        if (isset($province) && $province !== ''){
            ...
            $citys = Db::name('area')->where($where)->column('city_name','id');
            $this->success('', null, $citys);
        }else{
            $this->error('未传入有效参数');
        }
    }
    public function getBnb(){
        ...
        if (isset($city) && $city){
            $state = [config('state.state_ok'),config('state.state_mark')];
            $where = [
                'area_city_code'=> $city,
                'status' => ['in',$state]
            ];
            $bnbs = Db::name('bnb')->where($where)->column('name','id');
            $this->success('', null, $bnbs);
        }else{
            $this->error('未传入有效参数');
        }
    ...
    public function upload_img()
    {
        Config::set('default_return_type', 'json');
        $file = $this->request->file('file');
        $image_class_id = $this->request->param('image_class_id');
        $bnb_id = $this->request->param('bnb_id');
        $upload_type = $this->request->param('upload_type','upload');
        $name = $this->request->param('name','');
        if (empty($image_class_id))
        {
            $this->error("未指定图片类别");
        }
        if (empty($bnb_id))
        ...
        $image = new BnbImage();
        $image->loadImageByFile($file->getPath().'/'.$file->getFilename());
        $res = $image->save($upload_type);
        if ($res)
        {
            $url = count($res['thumb'])>0?$res['thumb'][0]:$res['image'];
            $params = array(
                'bnb_id'    => $bnb_id,
