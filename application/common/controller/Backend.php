...
class Backend extends Controller
{
    ...
    public function _initialize()
    {
       ...
       /* $controllername = strtolower($this->request->controller());*/
        $controllername = strtolower(preg_replace('/((?<=[a-z])(?=[A-Z]))/', '_',$this->request->controller()));
     ...
     * @return array
     */
    protected function buildparams($searchfields = null, $relationSearch = null,$data=[],$relationModel = null)
    {
        ...
        if($data){
            foreach($data as $k=>$v){
                if(strpos($k,'.') && in_array(substr($k,2),array_keys($filter))){
                    $filter[$k] = $v['value'];
                    if($v['key'] != '='){
                        $op[$k] = $v['key'];
                    }
                    unset($filter[substr($k,2)]);
                }else{
                    $filter[$k] = $v['value'];
                    if($v['key'] != '='){
                        $op[$k] = $v['key'];
        ...
        foreach ($filter as $k => $v) {
            ...
            switch ($sym) {
                case '=':
                case '!=':
                    $where[] = [$k, $sym, trim((string)$v)];
                    break;
                case 'LIKE':
                case 'NOT LIKE':
                case 'LIKE %...%':
                case 'NOT LIKE %...%':
                    $where[] = [$k, trim(str_replace('%...%', '', $sym)), "%".trim($v)."%"];
                    break;
                case '>':
                case '>=':
                case '<':
