...
var HotelDatepicker = (function () {
    ...
    HotelDatepicker.prototype.init = function init () {
        ...
        if (this.datelist != '')
        {
            var dl = JSON.parse(this.datelist);
            if (dl.length>=0)
            {
               this.startDate = dl[0].date;
               this.endDate = dl[dl.length-1].date;


               this.datelist_price = {};
               this.datelist_disabled = {};

               for(var i=0;i<dl.length;i++)
               {
                   this.datelist_price[dl[i].date] = dl[i].price;
                   if (dl[i].disabled) {
                       this.datelist_disabled[dl[i].date] = dl[i].disabled;
                   }
               }
            }
        }
    ...
    HotelDatepicker.prototype.createMonthDomString = function createMonthDomString (_date) {
        ...
        for (var week = 0; week < 6; week++) {
            ...
            for (var i$2 = 0; i$2 < 7; i$2++) {
                ...
                // false in this case.
                if (_day$2.valid && this$1.disabledDatesTime.length > 0) {

                    for(var i=0;i<this$1.disabledDatesTime.length;i++)
                    {
                        var v=  this.compareDay(_day$2.date, this$1.disabledDatesTime[i]);
                        if (v == 0)
                        {
                            _day$2.valid = false;
                            isDisabled = true;
                ...
                if (this.datelist)
                {
                    var dt = this$1.getDateString(_day$2.time, 'YYYY-MM-DD');
                    if((this.datelist_price[dt]) && (this.showprice))
                    {
                        _day$2.day = ""+_day$2.day +"<br/><span class='hotel_date'>￥"+this.datelist_price[dt]+"</span>";
                    }
                    else
                    {
                        _day$2.day = ""+_day$2.day;
                    }
                }
    ...
    HotelDatepicker.prototype.documentClick = function documentClick (evt) {
        ...
        } else if (evt.target.className.toLowerCase() === 'hotel_date')
            {
            this.dayClicked(evt.target.parentNode);
        }
    };
    ...
    HotelDatepicker.prototype.datepickerHover = function datepickerHover (evt) {
        ...
        } else if (evt.target.tagName && evt.target.className.toLowerCase() === 'hotel_date')
        {
            this.dayHovering(evt.target.parentNode);
        }
    ...
    HotelDatepicker.prototype.isValidDate = function isValidDate (time) {
        ...
        if (this.start && !this.end) {
            ...
            if (this.disabledDatesTime.length > 0) {
    ...
    HotelDatepicker.prototype.parseDisabledListDates = function parseDisabledListDates () {

        // Sort disabled dates and store it in property
        var _tmp =[];
        this.setFechaI18n();
        var i = 0;
        for (var key in this.datelist_disabled)
        {
            _tmp[i] = fecha.parse(key, 'YYYY-MM-DD');
            i++;
        }
        _tmp.sort(function (a, b) {
            return a - b;
