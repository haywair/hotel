...
;(function(factory) {
    ...
    new Rules({
        ...
        numberic: function(element, params) {
            var re, z = '0|',
                p = '^([0-9])[0-9]*(\\.\\w*)?$',
                key = params ? params[0] : '*';
            ...
            return new RegExp(re).test(this.value) || ('请输入数字');
        },
        mobile: function(element, params) {
            var p = '^1[34578]{1}\d{9}$';
            return new RegExp(p).test(this.value) || ('请输入11位手机号码');
        },
        idcard:function(element,params){
            var p = /(\d{15}$|^\d{18}$|^\d{17}(\d|X|x))$/;
            return new RegExp(p).test(this.value) || ('请输入身份证号码');
        },
