define(['jquery', 'bootstrap', 'upload', 'validator'], function ($, undefined, Upload, Validator) {
    var Form = {
        ...
        events: {
            ...
            bnbchooseimage: function (form) {
                //绑定fachoose选择附件事件
                if ($(".bnbchooseimage", form).size() > 0) {
                    $(document).on('click', ".bnbchooseimage", function () {
                        var that = this;
                        var multiple = $(this).data("multiple") ? $(this).data("multiple") : false;
                        var mimetype = $(this).data("mimetype") ? $(this).data("mimetype") : '';
                        var imagetype = $(this).data("imagetype") ? $(this).data("imagetype"): "";

                        parent.Fast.api.open("image_select/select?element_id=" + $(this).attr("id") + "&multiple=" + multiple + "&mimetype=" + mimetype + "&imagetype="+imagetype, __('Choose'), {
                            callback: function (data) {
                                var input_id = $("#" + $(that).attr("id")).data("input-id");
                                if (data.multiple) {
                                    var urlArr = [];
                                    ...
                                    urlArr.push(data.name);
                                    inputObj.val(urlArr.join(",")).trigger("change");
                                } else {
                                    $("#" + input_id).val(data.name).trigger("change");
                                }
                            }
                        });
                        return false;
