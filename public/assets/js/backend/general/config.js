define(['jquery', 'bootstrap', 'backend', 'table', 'form'], function ($, undefined, Backend, Table, Form) {
    ...
    var Controller = {
        ...
        config: function () {
            Controller.api.bindevent();
            Controller.api.setManageConfig();
        },
        api: {
            bindevent: function () {
                Form.api.bindevent($("form[role=form]"));
            },
            setManageConfig:function(){
                $('.btn').click(function(){
                    var field = $('#field').val();
                    var config_val = $('#manage_fee_percent').val();
                    var rule = /^[0-9]\d*(\.\d+)?$/;
                    if(!config_val){
                        Toastr.error('请填写平台管理费用比例');return false;
                    }else if(!rule.test(config_val)){
                        Toastr.error('请输入大于0的数字');return false;
                    }

                    Fast.api.ajax({url:'general/config/config',data:{field:field,value:config_val}});
                });
            }
        }
    };
